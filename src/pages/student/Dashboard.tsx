...
export default function StudentDashboard() {
  ...
  const quickLinks = [
    ...
    { path: "/student/marks", icon: FileText, label: "View Marks" },
    { path: "/student/timetable", icon: Clock, label: "Timetable" },
    { path: "/student/notices", icon: TrendingUp, label: "Notices" },
  ];
    ...
    <StudentLayout>
      ...
      <div data-pull-to-refresh className="h-full overflow-y-auto">
        <StaggeredContent className="space-y-4 sm:space-y-6 w-full max-w-full overflow-x-hidden">
          <StaggeredItem>
            <div className="relative overflow-hidden rounded-xl border border-border/60 bg-card/70 backdrop-blur-sm p-4 sm:p-6">
              <div className="relative z-10">
                <h1 className="text-lg sm:text-2xl md:text-3xl font-semibold mb-1 sm:mb-2 break-words text-foreground">
              ...
              <GraduationCap className="absolute right-2 sm:right-4 bottom-2 sm:bottom-4 h-16 w-16 sm:h-24 sm:w-24 text-muted/40" />
            </div>
          </StaggeredItem>

          <StaggeredItem>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 w-full">
              <Card className="border border-border/60 bg-card/70 min-w-0">
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <p className="text-[10px] sm:text-xs text-muted-foreground">Attendance</p>
                      <p className="text-lg sm:text-2xl font-semibold text-foreground">{stats.attendance}%</p>
                    </div>
                    <Calendar className="h-6 w-6 sm:h-8 sm:w-8 text-primary/70 flex-shrink-0" />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs sm:text-sm font-semibold text-foreground truncate">{profile.enrollment_number}</p>
                    </div>
                    <BookOpen className="h-6 w-6 sm:h-8 sm:w-8 text-primary/70 flex-shrink-0" />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs sm:text-sm font-semibold text-foreground truncate">{profile.student_id}</p>
                    </div>
                    <User className="h-6 w-6 sm:h-8 sm:w-8 text-primary/70 flex-shrink-0" />
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-lg sm:text-2xl font-semibold text-foreground">{stats.pendingNotices}</p>
                    </div>
                    <TrendingUp className="h-6 w-6 sm:h-8 sm:w-8 text-primary/70 flex-shrink-0" />
                  </div>
                </CardContent>
              </Card>
            </div>
                ...
                {quickLinks.map((link) => {
                  ...
                  return (
                    <Link key={link.path} to={link.path}>
                      <Card className="border border-border/60 bg-card/70 hover:bg-card/80 transition-colors cursor-pointer group min-w-0">
                        <CardContent className="p-2 sm:p-4 flex flex-col items-center text-center gap-1 sm:gap-2">
                          <div className="h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 rounded-full bg-primary/15 border border-primary/30 flex items-center justify-center">
          ...
          <StaggeredItem>
            <Card className="w-full border border-border/60 bg-card/70">
              ...
              <CardContent className="grid gap-2 sm:gap-3 md:gap-4 grid-cols-1 sm:grid-cols-2">
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg bg-muted/40 min-w-0">
                ...
                <div className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-lg bg-muted/40 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs sm:text-sm font-medium truncate">
                      {profile.course_name} - Y{profile.year} - {profile.section}
                    </p>
                  </div>
                </div>
              </CardContent>
